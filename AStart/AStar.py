...
def algoAround(array, start, goal, square=False, colorNuance=False, animation=[1, 1],delay = 0, distanceMax=2000, borderSize=1):
    ...
    while blExit == False:
        ...
        icpt += 1
        if icpt >= distanceMax:
            print("La distance max à été atteinte, réessayé en augmentant le DISTANCEMAX dans config.xml")
            blExit = True
...
def ReadConfigFile(namefile):
    tree = etree.parse(namefile)
    root = tree.getroot()
    config = []
    researchArray = ['SQUAREFIND','COLORTEINT','ANIMATION','ANIMATIONDELAY','DISTANCEMAX','BORDERSIZE','COLORDEP','COLOREND']
    for child in root:
        if child.tag == 'SQUAREFIND' or child.tag == 'COLORTEINT':
            config.append(bool(child.text))
        elif child.tag == 'ANIMATION':
            first = int(child.attrib['first'])
            second = int(child.attrib['second'])
            config.append([first,second])
        elif child.tag == 'ANIMATIONDELAY' or child.tag == 'DISTANCEMAX' or child.tag == 'BORDERSIZE':
            config.append(int(child.text))
        elif child.tag == 'COLORDEP' or child.tag == 'COLOREND':
            val = []
            for subChild in child:
                val.append(int(subChild.text))
            config.append((val[0], [val[1],val[2],val[3]]))
    return config

###### CONFIG ######
config = ReadConfigFile('config.xml')
...
if len(sys.argv) > 2:
    depPoint, endPoint = ReadPositionFile(sys.argv[2])
    img = cv.imread(sys.argv[1], cv.IMREAD_ANYCOLOR)
elif len(sys.argv) > 1:
    img = cv.imread(sys.argv[1], cv.IMREAD_ANYCOLOR)
    depPoint, endPoint = FindDepEnd(img, config[6], config[7])
...
if closeProgram:
    ...
    cv.waitKey()
    #finalPath = algoAround(imgDev, depPoint, endPoint, SQUAREFIND, COLORTEINT, ANIMATION, ANIMATIONDELAY, DISTANCEMAX, BORDERSIZE)
    finalPath = algoAround(imgDev, depPoint, endPoint, config[0], config[1], config[2], config[3],config[4],config[5])
    imgFinal = img.copy()
